...
for (tissue in c('Blood', 'LesionalSkin', 'NonlesionalSkin')) {
  ...
  # SVA
  mod <- model.matrix(~ 0 + Time + Sex + Age + BMI + PASI_A + Time:DeltaPASI,
                      data = pheno)
  mod0 <- model.matrix(~ 0 + Time + Sex + Age + BMI + PASI_A, 
                       data = pheno)
  ...
  fwrite(top, paste0(getwd(), '/Results/voom/', 
         tissue, ',wk00.txt'), sep = '\t')
