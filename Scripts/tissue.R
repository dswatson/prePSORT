...
# SVA
mod <- model.matrix(~ Subject + Time + Site, data=pheno)
...
res <- data.frame(results(dds, contrast=c('Site', 'B', 'L'), filterfun=ihw))
id <- rownames(res)
res <- res %>%
  mutate(gene_id = id) %>%
...
fwrite(res, paste0(getwd(), '/Results/DESeq/BvsL.txt'), sep='\t')

# Blood vs. NonlesionalSkin
res <- data.frame(results(dds, contrast=c('Site', 'B', 'N'), filterfun=ihw))
id <- rownames(res)
res <- res %>%
  mutate(gene_id = id) %>%
  inner_join(e2g, by='gene_id') %>%
  ...
  arrange(p.value) %>%
  select(EnsemblID, GeneSymbol, AvgExpr, logFC, p.value, q.value)
fwrite(res, paste0(getwd(), '/Results/DESeq/BvsN.txt'), sep='\t')

# LesionalSkin vs NonlesionalSkin
res <- data.frame(results(dds, contrast=c('Site', 'L', 'N'), filterfun=ihw))
id <- rownames(res)
res <- res %>%
  mutate(gene_id = id) %>%
  inner_join(e2g, by='gene_id') %>%
  ...
  arrange(p.value) %>%
  select(EnsemblID, GeneSymbol, AvgExpr, logFC, p.value, q.value)
fwrite(res, paste0(getwd(), '/Results/DESeq/LvsN.txt'), sep='\t')
